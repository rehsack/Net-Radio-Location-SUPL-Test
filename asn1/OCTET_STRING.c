...
int
BCD_OCTET_STRING_fromBuf(OCTET_STRING_t *st, const char *str, int len) {
	uint8_t *buf;
	ssize_t siz, i;
...
ssize_t
OCTET_STRING_toBuf(OCTET_STRING_t *s, uint8_t *buf, ssize_t bufsiz)
{
	if(!s || !buf)
		return -EINVAL;
...
ssize_t
BCD_OCTET_STRING_toBuf(OCTET_STRING_t *s, uint8_t *buf, ssize_t bufsiz)
{
	ssize_t i;
...
ssize_t
OCTET_STRING_toNewBuf(OCTET_STRING_t *s, uint8_t **buf)
{
	if(!s || !buf)
		return -EINVAL;
	...
	*buf = MALLOC(s->size);
	...
	return OCTET_STRING_toBuf(s, *buf, s->size);
}

ssize_t
BCD_OCTET_STRING_toNewBuf(OCTET_STRING_t *s, uint8_t **buf)
{
	if(!s || !buf)
		return -EINVAL;
	...
	*buf = MALLOC(s->size * 2);
	...
	return BCD_OCTET_STRING_toBuf(s, *buf, s->size * 2);
}

ssize_t
OCTET_STRING_toStr(OCTET_STRING_t *s, char *str, ssize_t strsiz)
{
	if(!s || !str)
		return -EINVAL;
...
ssize_t
BCD_OCTET_STRING_toStr(OCTET_STRING_t *s, char *str, ssize_t strsiz)
{
	ssize_t copied = BCD_OCTET_STRING_toBuf(s, (uint8_t *)str, strsiz - 1);
	if(copied >= 0) {
		str[copied] = '\0';
		++copied;
	}
...
ssize_t
OCTET_STRING_toNewStr(OCTET_STRING_t *s, char **str)
{
	ssize_t copied;
	...
	*str = MALLOC(s->size + 1);
	...
	copied = OCTET_STRING_toBuf(s, (uint8_t *)(*str), s->size);
	if(copied >= 0) {
		(*str)[copied] = '\0';
		++copied;
	}
...
ssize_t
BCD_OCTET_STRING_toNewStr(OCTET_STRING_t *s, char **str)
{
	ssize_t copied;
	...
	*str = MALLOC(s->size * 2 + 1);
	if(!*str)
		return -ENOMEM;

	copied = BCD_OCTET_STRING_toBuf(s, (uint8_t *)(*str), s->size * 2);
	if(copied >= 0) {
		(*str)[copied] = '\0';
		++copied;
	}
